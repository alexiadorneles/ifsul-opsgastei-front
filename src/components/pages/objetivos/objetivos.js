...
class Objetivos extends Component {
	...
	renderObjetivosIncompletos() {
		...
		return objetivosIncompletos.map(o => (
			<OpsGasteiObjetivoIncompleto key={o.nome} objetivo={o} callbackAcao={this.buscarObjetivos} />
		))
	}

	renderObjetivosCompletos() {
		const { objetivosCompletos } = this.state
		return objetivosCompletos.map(o => (
			<OpsGasteiObjetivoCompleto key={o.nome} objetivo={o} callbackAcao={this.buscarObjetivos} />
		))
	}
		...
		return (
			<SecuredContainer>
				...
				<View style={STYLES.objetivosContainer}>
					...
					<OpsGasteiBox title="Gastos Fixos">{this.renderGastosFixos()}</OpsGasteiBox>
