...
import OpsGasteiLegendaGrafico from 'components/generic/ogLegendaGrafico'
...
class Estatistica extends Component {
  ...
  buscarObjetivos() {
    ...
    this.seriesObjetivos = Object.values(agrupados)
  ...
  buscarGastosFixos() {
    ...
    this.seriesGastos = Object.values(agrupados)
      .map(arr => arr.map(obj => obj.valor).reduce((a, b) => a + b, 0))
    this.nomes = []
    Object.keys(agrupados).forEach(nomeCat => {
