...
export default class OutroMesCtrl {
  constructor(categoriaService, gastoFixoService, salarioService,
    objetivoService, swalService, $localStorage, $location, $routeParams) {
    ...
    this.buscarSalario(ano, mes).then(response => {
      if (!response.data) {
        ...
        swalService.info(`Você foi redirecionado à tela inicial pois não possui dados
          no mês ${moment(this.mes).locale('pt-br').format('MMMM').replace(/\b\w/g, l => l.toUpperCase())} de ${moment(this.mes).year()}`)
        return
      }
