...
import STYLES from './ogSaldoStyle'
...
class OpsGasteiSaldo extends Component {
  ...
  renderSalario() {
    if (this.state.salario) {
      return <Text style={STYLES.valorSalario}>Salário: {this.state.salario}</Text>
    }
  }
  ...
  render() {
    return (
      <View style={this.state.valorSaldo < 0 ? STYLES.saldoNegativo : STYLES.saldoPositivo}>
        <Text style={STYLES.valorSaldo}>{this.state.valorSaldoExibicao}</Text>
        {this.renderSalario()}
      </View>
    )
  }
